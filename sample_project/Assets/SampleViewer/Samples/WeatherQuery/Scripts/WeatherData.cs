...
public class WeatherData : MonoBehaviour
{
    ...
    public void SetSky()
    {
        if (volumeProfile.TryGet(out vClouds))
        {
            ...
            {
                vClouds.cloudPreset =
                    new VolumetricClouds.CloudPresetsParameter(VolumetricClouds.CloudPresets.Overcast);
                directionalLight.color = new Color(0.1803922f, 0.1803922f, 0.1803922f, 1.0f);
            }
            else if (skyCondition.ToLower().Contains("cloud"))
            {
                vClouds.cloudPreset =
                    new VolumetricClouds.CloudPresetsParameter(VolumetricClouds.CloudPresets.Cloudy);
                directionalLight.color = new Color(1, 1, 1, 1);
            }
            else if (currentWeather.ToLower().Contains("thunder"))
            { 
                vClouds.cloudPreset =
                new VolumetricClouds.CloudPresetsParameter(VolumetricClouds.CloudPresets.Stormy);
            ...
            {
                vClouds.cloudPreset =
                    new VolumetricClouds.CloudPresetsParameter(VolumetricClouds.CloudPresets.Sparse);
                directionalLight.color = new Color(1, 1, 1, 1);
            }
        }
    }
